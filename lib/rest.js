...
function handleGet(req, res, next) {
  ...
  if (req.params.id) {
    ...
    query = req.query.query
      ? util.parseJSON(req.query.query, next, restify)
      : {};
  }
  ...
  MongoClient.connect(util.connectionURL(req.params.db, config), function(
    err,
    db
  ) {
    ...
    MongoClient.connect(util.connectionURL(req.params.db, config), function(
      ...
      collection.insert(
        Array.isArray(req.body)
          ? util.cleanParams(req.body[0])
          : util.cleanParams(req.body),
        function(err, docs) {
          res.header(
            "Location",
            "/" +
...
server.put("/:db/:collection/:id", function(req, res) {
  ...
  MongoClient.connect(util.connectionURL(req.params.db, config), function(
    ...
    db.collection(req.params.collection, function(err, collection) {
      collection.update(spec, util.cleanParams(req.body), true, function(
        err,
        docs
      ) {
        res.set("content-type", "application/json; charset=utf-8");
